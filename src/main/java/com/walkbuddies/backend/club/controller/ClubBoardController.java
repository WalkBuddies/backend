...
public class ClubBoardController {
  ...
  //create
  @PostMapping("/write")
  public ResponseEntity<SingleResponse<ClubBoardDto>> createBoard(
  ...
  public ResponseEntity<PageResponse<Page<ClubBoardDto>>> boardList(
      @PageableDefault(size = 20, sort = "clubBoardId", direction = Sort.Direction.DESC) Pageable pageable,
      @RequestBody ClubBoardListDto listDto) {
    Page<ClubBoardDto> data;


    data = clubBoardService.postList(pageable, listDto.getClubId(), listDto.getType(), listDto.getKeyword(), 0);
  ...
  @PostMapping("/update")
  public ResponseEntity<SingleResponse<ClubBoardDto>> updateBoard(
      @RequestPart(value = "board") ClubBoardDto clubBoardDto,
      @RequestPart(value = "files", required = false) List<Long> fileId) {
  ...
  public ResponseEntity<SingleResponse<String>> deleteBoard(@PathVariable long boardId) {
    ...
    SingleResponse<String> result = new SingleResponse<>(HttpStatus.OK.value(), "삭제 완료", null);
